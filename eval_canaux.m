...
for k=1:nb_canaux
    ...
    for i=1:nb_trame_test %On envoit 30 trames de test
        ...
        [suite_bits_recu,x_recu]=demodulationDMT(x_mod,ones(1, nb_canaux), nb_canaux,pref_cycl,vect_alloc);
        disp('symboles recus');
        disp(x_recu);
...
%Calcule du bruit
    ...
    for i=1:nb_trame_test
        bruit(k)=bruit(k) + (x_recu_total(i)-H_moy(k)*suite_symb_QAM(k));
